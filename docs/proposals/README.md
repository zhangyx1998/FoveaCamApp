...
| [viewer-timeline-touchup](./viewer-timeline-touchup.md) | shipped; eyeball pass owed | 8-item viewer touch-up: first-meaningful initial playhead, per-track tile slots with drag order, click-to-seek ruler, track theme colors, cursor-anchored time zoom + bouncy pan with bleed, smooth playhead, borderless-inline-buttons design rule, between-track block insertion |
| [viewer-tiles-split-and-project](./viewer-tiles-split-and-project.md) | shipped; eyeball pass owed | tiles fill panel width with divider percentage-resize (no scroll); track theme color across chip/outline/projection; instant overscroll settle; projectable tiles via a ref-counted `{kind:"viewer"}` BroadcastChannel mirror source |
| [split-tracking-app](./split-tracking-app.md) | shipped; rig owed | new app: per-eye independent visual servos — drop a target on each fovea view (PosView), switchable KCF/hybrid tracker on a 512² tile, each mirror keeps its target centered; capture + recording |
