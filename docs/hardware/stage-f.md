...
- [ ] **PosView steers the mirror** — dragging the LEFT voltage pad moves the
  left mirror (manual steering) and never perturbs the right; the pad marker
  rides the servo-driven volt between drags. `:lim` is the controller voltage
  envelope (`controller.dv`), not a pixel range.
- [ ] **Per-side independence + reseed** — steer the target into the LEFT
  center box, release → the left tracker locks the center tile and the mirror
  follows; grabbing the pad again stops that side; the right side is untouched
  throughout.
- [ ] **Track & center** — after release, each mirror keeps its locked target
  at the fovea frame center under target motion; `tracking.<eye>` reads true;
  losing the target (occlusion) holds the mirror and flips `blocked`.
- [ ] **Tracker swap** — Hybrid↔KCF in the drawer re-arms live at the center
  tile with no glitch; the drawer never shows an engine that isn't running.
- [ ] **Tile annotation** — the box is drawn FIXED at the frame center, its
  label reads the configured size, and it matches the actual `arm()` ROI;
  changing the tile size re-arms both armed sides at center.
