...
Behavior spec for `core/include/PortPipe.h` + `core/src/PortPipe.cpp`. This file
records the runtime contract; the code carries `// spec:` pointers to the anchors
below.
...
endpoints are native C++ threads; the iterator remains only where JS
...
blocked on a stalled upstream. `LinkStats.held`
reports whether the slot currently pins a payload — true only between a write and
its readout; a drained link on a stalled upstream reads false. The `held` probe
guards against a link pinning a payload on a stalled upstream.
