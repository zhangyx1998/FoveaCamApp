...
Behavior spec for the Aravis frame-processing bricks. This file records the
runtime invariants the code carries `// spec:` pointers to.

## converter {#converter}

`ConverterStream` (`core/lib/Aravis/ConverterStream.h`) is the per-stream format
converter thread. A dedicated
...
`OwnedFrame` is the OWNED element type of the
...
`RawPipe` (`core/lib/Aravis/RawPipe.cpp`) applies
...
for the whole push). CONSUMER-GATED: no recorder/capture attached → the
...
raw12p pipes) taps the ArvBuffer BEFORE Frame
...
**latest-wins mode**: both inputs are
...
**paired mode**: the SGBM join over
...
On-demand like the latest-wins brick (parks with no consumers → the record
tap unsubscribes; the pair brick's keep-alive is unaffected).

**compute**: BGRA→GRAY both sides, then a
