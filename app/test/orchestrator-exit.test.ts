// Exit-decision logic for the orchestrator lifecycle. The clean/crash decision
// must be ACK-based — never
...
describe("classifyOrchestratorExit", () => {
  ...
  it("ack present ⇒ clean even with a NON-zero code (code is never the discriminator)", () => {
    // The case a naive `code === 0` check gets wrong: a graceful exit
