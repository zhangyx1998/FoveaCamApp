...
function getRect(): Partial<DOMRect> {
    return (navigator as any).windowControlsOverlay?.getTitlebarAreaRect?.() ?? {} as Partial<DOMRect>;
}

const rect = ref<Partial<DOMRect>>(getRect());

const height = computed(() => (rect.value?.height ?? 40) + (rect.value?.top ?? 0));
