...
template <SmartPtrLike T> class Stream {
  ...
  bool auto_start = true, auto_stop = true;
...
protected:
  ...
  void unsubscribe(Subscriber<T> *subscriber) {
    unsubscribe(subscriber, std::unique_lock<std::mutex>(mutex));
  };

  void unsubscribe(Subscriber<T> *subscriber,
                   std::unique_lock<std::mutex> &&lock) {
    subscribers.erase(subscriber);
    if (subscribers.empty() && auto_stop && flag_active) {
      flag_active = false;
      lock.release();
      // Wait for loop stop.
      std::scoped_lock loop_lock(loop_mutex);
    }
  };
  ...
  inline bool __activate__() { return flag_active || flag_term; }
